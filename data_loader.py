...
class Custom_Mnist_Dataset(data.Dataset):
    ...
    def __init__(self, root, train=True, transform=None, target_transform=None, refresh=False):
        self.noise = .5
    ...
    def download(self):
        ...
        h = read_label_file(os.path.join(self.root, self.raw_folder, 'train-labels-idx1-ubyte'))
        ...
        training_set = (
            ...
            torch.from_numpy(huy)
        )

        h = read_label_file(os.path.join(self.root, self.raw_folder, 't10k-labels-idx1-ubyte'))
        u = np.random.randint(0,10,h.shape)
        y = np.round(np.clip(h + u + np.random.normal(0,self.noise,h.shape), 0,18)/3)
        u = np.clip(u/9, self.eps, 1-self.eps)
        y = np.clip(y/6, self.eps, 1-self.eps)
